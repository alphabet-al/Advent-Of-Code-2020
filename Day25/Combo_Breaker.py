...
You don't have enough stars to pay the deposit, though. You need 11 more.
...
def transform(pval, loop_size):
    val = 1

    for i in range(loop_size):
        val = (val*pval) % 20201227
    
    return val


def code_breaker(data):

    card_loop = rev_transform(data[0])
    door_loop = rev_transform(data[1])

    encrypt_key = transform(data[1], card_loop)
...
if __name__ == "__main__":
    ...
    print('Encryption Key: {}'.format(code_breaker(data)))