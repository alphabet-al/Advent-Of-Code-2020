...
--- Part Two ---
The final step in breaking the XMAS encryption relies on the invalid number you just found: you must find a contiguous set of at least two numbers in your list which 
...
309
576
In this list, adding up all of the numbers from 15 through 40 produces the invalid number from step 1, 127. (Of course, the contiguous set of numbers in your actual 
...
def any_two_equal_to(numbers, k):
    return any(((i + j) == k ) for i, j in combinations(numbers, 2))

def search(lst, preamble_length):
    number_check = True
    start = 0
    end = preamble_length
    k_index = preamble_length

    while number_check:
        numbers = lst[start:end]
        k = lst[k_index]

        if any_two_equal_to(numbers, k):
            start += 1
            end += 1
            k_index += 1
        else:
...
def binary_search(lst, val):   
    ...
    for i in range(len(lst) - 1):        
        ...
        if sum(lst[mini:maxi]) < val:
            maxi += 1

        while sum(lst[mini:maxi]) > val:
            mini += 1

        if sum(lst[mini:maxi]) == val:
            return (min(lst[mini:maxi]) + max(lst[mini:maxi]))


if __name__ == "__main__":
    
    input = r'C:\Users\alanv\PythonCode\Projects\Advent of Code 2020\Day9\input.txt'
    ...
    # print('Invalid number: {0}'.format(search(data, 25)))
    print('Total of min/max: {0}'.format(binary_search(data, 1398413738)))
    # print('Total of min/max: {0}'.format(binary_search(test_data, 127)))
